...
public class GyroscopeControl : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        rawGyroRotation.position = rb.transform.position;
        rawGyroRotation.rotation = rb.transform.rotation;
    }
    ...
    private void FixedUpdate()
    {
        if (Time.timeScale == 1 && GyroEnabled)
        {
            ...
            rb.transform.rotation = Quaternion.Slerp(rb.transform.rotation, initialRotation * rawGyroRotation.rotation, smoothing); // Progressive rotation of the object
        }
    }
    ...
    void OnGUI () {
        if(debug){
            ...
            GUILayout.Label("Rotation: " + rb.transform.rotation.ToString(), style);
